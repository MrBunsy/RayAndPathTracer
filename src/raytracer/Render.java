...
public class Render {
    ...
    private int width, height, sliceWidth;
...
//            }
    ...
    //this just sends out a ray and finds the colour for this ray, taking into account the whole world and all the rules
    //find the colour of the pixel on this ray
    //ignore objects in ignore
    //if inside is true we are currently inside insideShape
    public Colour getPixel(Ray ray, int iteration, double currentN, double currentPower) {//ArrayList<Shape> ignore,//iteration 5 by default, ignore empty and inside false
        ...
        if(collision.collide){
            ...
            //if (!inside) {
                ...
                if(world.isSunlight()){
                    ...
                    if(isSunlit(world,collisionPoint,collision.with)){
...
//                    }
                    ...
                    if(isLit(world,collision.with,lightRay)){
            ...
            if (surface.reflective > 0) {//!inside && 
                ...
                Colour reflectedColour = getPixel(reflectedRay, iteration - 1,  currentN, currentPower*surface.reflective);//inSceneArray,
            ...
            if (surface.clear > 0) {
                ...
                if(R>0.002){
                    ...
                    Colour reflectedColour = this.getPixel(reflectedRay, iteration - 1,  n1, currentPower*R);//ignore,
                ...
                if(!tir && T > 0.002){
                    ...
                    Colour refractedColour = this.getPixel(refractedRay, iteration - 1,  n2, currentPower*T);//ignore,
    ...
    //if less than the max number of threads are currently rendering, set more going
    private void fillEmptyThreads(){
        try {
    ...
    public void render(Graphics2D _g, int section, int totalSections) {//int width,int height,


        int sectionWidth = images[section].getWidth();//sliceWidth;
        
//        if(section==totalSections-1){
//            sectionWidth += width - totalSections*sliceWidth;
//        }
        ...
        for (int x = 0; x < sectionWidth; x++) {
            for (int y = 0; y < height; y++) {
                ...
                Colour colour = getPixel(ray, world.iterate,  world.getAirN(), 1.0);

                //_g.setColor(colour.toColor());
                //_g.fillRect(x+sectionWidth*section, y, 1, 1);
                renderPixel(_g, colour, x, y);
                
                
            }
            
            //if(x*height - pixelsSoFar > 10000){
             //       pixelsSoFar+=x*height - pixelsSoFar;
    ...
    //a thread has rendered this many pixels
    ...
    public synchronized void renderPixel(Graphics2D _g, Colour colour, int x, int y) {
        _g.setColor(colour.toColor());
        _g.fillRect(x, y, 1, 1);
    }
...
class RenderThread implements Runnable {
    ...
    @Override
    public void run() {
        render.render(graphics, thisThread, threads);
